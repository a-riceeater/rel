...
async function interp(file, d) {
    try {

        console.log(file, d)

        /*if (d) uconsole = file;
        if (d && !file.toString().trim()) return*/

        const fd = d ? file : fs.readFileSync(file, "utf8")
        const flines = fd.split("\n");

        const mainFound = await checkForMain(file);
        mainFound ? handlers.wl("Main function found") : errors.throwMainNotFound(file.replace(/\.[^/.]+$/, ""), file, uconsole);
        if (!mainFound) return;

        const mainEnds = await manager.checkForEnd(file.replace(/\.[^/.]+$/, ""), fd);
        if (!mainEnds) errors.throwUED(file.replace(/\.[^/.]+$/, ""), file, uconsole)
        ...
        async function ia(start) {
            ...
            for (let i = start; i < flines.length; i++) {
                ...
                if (line == "") continue;
                    ...
                    if (!ended) errors.throwUED(readingFunction, file, uconsole);
                }
                ...
                /*else if (executingFunction) {
                    ...
                    if (o1 == "true") {
                        ...
                        iffs.push(i)
                        trueIffs.push(i)
                        if (executingFunction == "") errors.throwTypeError("()", i, file, uconsole)

                        if (!checkVoid(ofunc.trim(), flines, i)) errors.throwIFNotFound(ofunc.trim(), uconsole)
                        return ia(i);
                    }
                    ...
                    if (pastMain) errors.throwIllegalUsing(file, i, uconsole)
                    ...
                    if (executingFunction != line.substring(0, line.indexOf("(")).trim()) {
                        if (!checkVoid(line.substring(0, line.indexOf("(")).trim().trim(), flines, i)) errors.throwIFNotFound(line.substring(0, line.indexOf("(")).trim().trim(), uconsole)
                ...
                else errors.throwTypeError(line, i, file, uconsole)
